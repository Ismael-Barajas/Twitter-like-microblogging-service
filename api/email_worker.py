...
with greenstalk.Client(('127.0.0.1', 11300), watch="email") as client:
    while True:
        job = client.reserve()
        data = job.body.split()
        # data[0] is its source, data[1] is the email, data[2:] is every thing after
        fromAddress = f"twitter@email.com"
        toAddress = f"{data[1]}".split()
        msg = ("From: %s\r\nTo: %s\r\n\r\n" %
               (fromAddress, ", ".join(toAddress)))
        if data[0] == "poll":
            msg += f"Your post contained an invalid poll link: '{' '.join(data[2:])}', we went ahead and deleted your post.  Feel free to try again with a valid link to the poll. Sorry for the inconvenience."
        elif data[0] == "like":
